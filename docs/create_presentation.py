...
def _apply_gradient(shape, stops, angle_deg=90.0):
    ...
    for tag in [qn("a:solidFill"), qn("a:gradFill"), qn("a:noFill"),
        ...
        if el is not None:
            spPr.remove(el)
    gs_xml = "".join(
        f'<a:gs pos="{int(p*100000)}"><a:srgbClr val="{c}"/></a:gs>'
        for p, c in stops
    )
...
def txt(slide, text, left, top, width, height,
        size=16, bold=False, color=CGI_BODY,
...
def cornerstone(slide):
    grad_rect(slide, Inches(6.80), Inches(0),   Inches(6.53), Inches(3.90),
              [(0.0,"FF8C00"),(1.0,"E4002B")], 135)
    grad_rect(slide, Inches(10.00), Inches(3.90), Inches(3.33), Inches(3.60),
              [(0.0,"E4002B"),(1.0,"8C0015")], 90)
...
def add_footer(slide, page_num):
    ...
    txt(slide, "Internal", Inches(5.5), y, Inches(2.33), h,
        size=10, color=CGI_GREY, align=PP_ALIGN.CENTER)
    txt(slide, str(page_num), SLIDE_W - Inches(1.3), y, Inches(0.9), h,
        size=10, color=CGI_GREY, align=PP_ALIGN.RIGHT)


def slide_header(slide, title, subtitle=None):
    txt(slide, title,
        MARGIN_L, Inches(0.18), CONTENT_W, Inches(0.68),
        size=26, bold=False, color=CGI_BLACK)
    if subtitle:
        rect(slide, MARGIN_L, Inches(0.93), CONTENT_W, Inches(0.022), CGI_MID_GREY)
        txt(slide, subtitle,
            MARGIN_L, Inches(0.97), CONTENT_W, Inches(0.34),
            size=13, color=CGI_GREY)
...
def slide_01_cover(prs):
    ...
    txt(slide, "TDGen Temporal",
        Inches(0.55), Inches(1.40), Inches(6.0), Inches(0.90),
        size=42, bold=True, color=CGI_BLACK)
    txt(slide, "Day-Over-Day Synthetic Test Data Generation",
        Inches(0.55), Inches(2.38), Inches(6.0), Inches(0.55),
        size=22, bold=False, color=CGI_BODY)
    ...
    ], Inches(0.55), Inches(3.18), Inches(6.0), Inches(1.10), space_pt=5)
...
def slide_02_what(prs):
    ...
    # Title + rule
    txt(slide, "Temporal Test Data Generator (TDGen Temporal)",
        MARGIN_L, Inches(0.18), CONTENT_W, Inches(0.68),
        size=26, bold=False, color=CGI_BLACK)
    rect(slide, MARGIN_L, Inches(0.93), CONTENT_W, Inches(0.022), CGI_MID_GREY)
    ...
    # ------------------------------------------------------------------ #
    Y = Inches(1.04)
    txt(slide, "PURPOSE", MARGIN_L, Y, LEFT_W, Inches(0.24),
        size=10, bold=True, color=CGI_RED)
    Y += Inches(0.27)
    ...
    multi_para(slide, [
        ("\u2022  Enable CGI QE teams at TD Bank to generate high-fidelity synthetic test data on demand "
         ...
         "coordinating with source system teams.", 12, False, CGI_BODY),
        ("\u2022  TDGen Temporal is a standalone Python CLI with persistent SQLite state. "
         ...
         "The MCP server is an optional interaction layer.", 12, False, CGI_BODY),
    ], MARGIN_L, Y, LEFT_W, Inches(1.12), space_pt=5)
    Y += Inches(1.18)

    # ------------------------------------------------------------------ #
    # APPROACH  (5-step pipeline)
    # ------------------------------------------------------------------ #
    txt(slide, "APPROACH", MARGIN_L, Y, LEFT_W, Inches(0.24),
        size=10, bold=True, color=CGI_RED)
    Y += Inches(0.27)
    ...
    for idx, (color, icon, title, sub) in enumerate(steps):
        ...
        rect(slide, int(icon_x), Y + Inches(0.12), ICON_S, ICON_S,
             RGBColor(0xFF, 0xFF, 0xFF))
        txt(slide, icon,
            int(icon_x), Y + Inches(0.12), ICON_S, ICON_S,
            size=20, bold=True, color=color, align=PP_ALIGN.CENTER)
        txt(slide, title,
            sx + Inches(0.08), Y + Inches(0.82),
            STEP_W - Inches(0.16), Inches(0.36),
            size=12, bold=True, color=CGI_WHITE)
        txt(slide, sub,
            sx + Inches(0.08), Y + Inches(1.20),
            STEP_W - Inches(0.16), Inches(0.50),
            size=10, color=RGBColor(0xBB, 0xBB, 0xBB))
        sx += STEP_W
        if idx < 4:
            txt(slide, "\u203a",
                sx + Inches(0.01), Y + STEP_H / 2 - Inches(0.18),
                CHEV_W + Inches(0.02), Inches(0.36),
                size=18, bold=True, color=CGI_GREY, align=PP_ALIGN.CENTER)
            sx += CHEV_W
    ...
    # KEY OUTCOMES
    # ------------------------------------------------------------------ #
    txt(slide, "KEY OUTCOMES", MARGIN_L, Y, LEFT_W, Inches(0.24),
        size=10, bold=True, color=CGI_RED)
    Y += Inches(0.27)
    ...
    for main, sub in outcomes:
        ...
        txt(slide, main, ox + Inches(0.10), Y + Inches(0.07),
            OUT_W - Inches(0.16), Inches(0.38),
            size=12, bold=True, color=CGI_WHITE)
        txt(slide, sub,  ox + Inches(0.10), Y + Inches(0.48),
            OUT_W - Inches(0.16), Inches(0.30),
            size=10, color=CGI_GREY)
    ...
    txt(slide, "Tool Stack:",
        RIGHT_X + Inches(0.14), CARD_Y + Inches(0.14),
        RIGHT_W - Inches(0.22), Inches(0.38),
        size=14, bold=True, color=CGI_BLACK)
    ...
    multi_para(slide,
               ...
               RIGHT_X + Inches(0.14), CARD_Y + Inches(0.58),
               RIGHT_W - Inches(0.22), CARD_H - Inches(0.70),
               space_pt=3.5)
...
def slide_03_scope(prs):
    slide = blank_slide(prs)
    slide_header(slide, "POC Scope  \u2014  Data Model & Simulation Engine",
    ...
    txt(slide, "DATA MODEL",
        MARGIN_L + Inches(0.14), PANEL_TOP + Inches(0.12),
        LEFT_W - Inches(0.20), Inches(0.22),
        size=10, bold=True, color=CGI_RED)
    txt(slide, "TSYS TS2  \u2014  publicly available approximation, built by Claude",
        MARGIN_L + Inches(0.14), PANEL_TOP + Inches(0.38),
        LEFT_W - Inches(0.20), Inches(0.28),
        size=12, bold=False, color=CGI_BLACK)
    ...
    for num, label in stats:
        ...
        txt(slide, num,
            tx, ty + Inches(0.18), TILE_W, Inches(0.90),
            size=32, bold=True, color=CGI_WHITE, align=PP_ALIGN.CENTER)
        txt(slide, label,
            tx, ty + Inches(1.14), TILE_W, Inches(0.44),
            size=9, color=RGBColor(0xBB, 0xBB, 0xBB), align=PP_ALIGN.CENTER)
        tx += TILE_W + TILE_GAP
    ...
    txt(slide, "ENTITY STATE MACHINES",
        RIGHT_X + Inches(0.14), PANEL_TOP + Inches(0.12),
        RIGHT_W - Inches(0.20), Inches(0.22),
        size=10, bold=True, color=CGI_RED)
    txt(slide, "6 entities advance through defined lifecycle states each simulation day",
        RIGHT_X + Inches(0.14), PANEL_TOP + Inches(0.38),
        RIGHT_W - Inches(0.20), Inches(0.28),
        size=12, bold=False, color=CGI_BLACK)
    ...
    machines = [
        ...
        ("FRAUD ALERT",
         "OPEN \u2192 UNDER_REVIEW \u2192 CONFIRMED | FALSE_POS",
         CGI_ORANGE),
        ("CHARGEBACK",
    ...
    for name, states, color in machines:
        ...
        txt(slide, name,
            RIGHT_X + Inches(0.24), sy + Inches(0.08),
            NAME_W, Inches(0.24),
            size=11, bold=True, color=color)
        txt(slide, states,
            RIGHT_X + Inches(0.24) + NAME_W, sy + Inches(0.10),
            RIGHT_W - Inches(0.28) - NAME_W - Inches(0.10), Inches(0.22),
            size=10, color=CGI_BODY)
        sy += SM_H + SM_GAP
    ...
    txt(slide, "DAILY SIMULATION LOOP",
        MARGIN_L + Inches(0.14), BOTTOM_Y + Inches(0.12),
        CONTENT_W - Inches(0.22), Inches(0.22),
        size=10, bold=True, color=CGI_RED)
    ...
    txt(slide, loop_text,
        MARGIN_L + Inches(0.14), BOTTOM_Y + Inches(0.40),
        CONTENT_W - Inches(0.22), BOTTOM_H - Inches(0.50),
        size=12, color=CGI_BODY)
...
def slide_03_how(prs):
    ...
    steps = [
        ...
        ("2", "Account state machine",   "Balance updates, delinquency aging, ACTIVE->DELINQUENT->CHARGEOFF"),
    ...
    for i, (num, heading, detail) in enumerate(steps):
        ...
        txt(slide, num,
            x + Inches(0.10), y + Inches(0.05),
            Inches(0.30), Inches(0.28),
            size=13, bold=True, color=CGI_RED)
        txt(slide, heading,
            x + Inches(0.42), y + Inches(0.05),
            Inches(1.60), Inches(0.28),
            size=12, bold=True, color=CGI_BLACK)
        txt(slide, detail,
            x + Inches(2.08), y + Inches(0.05),
            col_w - Inches(2.16), Inches(0.28),
            size=12, color=CGI_BODY)
...
def slide_04_state_machines(prs):
    slide = blank_slide(prs)
    slide_header(slide, "Entity State Machines",
    ...
    for name, states, transitions in machines:
        ...
        txt(slide, name,
            X + Inches(0.12), TOP + Inches(0.04),
            BOX_W - Inches(0.16), Inches(0.36),
            size=15, bold=True, color=CGI_WHITE)

        # States
        sy = TOP + Inches(0.52)
        txt(slide, "States",
            X + Inches(0.12), sy, BOX_W - Inches(0.16), Inches(0.24),
            size=11, bold=True, color=CGI_GREY)
        sy += Inches(0.26)
        for state in states:
            rect(slide, X + Inches(0.12), sy, BOX_W - Inches(0.24), STATE_H,
                 RGBColor(0xFF,0xFF,0xFF))
            rect(slide, X + Inches(0.12), sy, Inches(0.04), STATE_H, CGI_BLUE)
            txt(slide, state,
                X + Inches(0.22), sy + Inches(0.04),
                BOX_W - Inches(0.36), Inches(0.28),
                size=12, bold=False, color=CGI_BLACK)
            sy += STATE_H + Inches(0.04)

        # Transitions
        sy += Inches(0.06)
        txt(slide, "Transitions",
            X + Inches(0.12), sy, BOX_W - Inches(0.16), Inches(0.24),
            size=11, bold=True, color=CGI_GREY)
        sy += Inches(0.26)
        for fr, to, cond in transitions:
            txt(slide, f"{fr} -> {to}",
                X + Inches(0.12), sy, BOX_W - Inches(0.16), Inches(0.20),
                size=11, bold=True, color=CGI_RED)
            sy += Inches(0.22)
            txt(slide, cond,
                X + Inches(0.18), sy, BOX_W - Inches(0.22), Inches(0.20),
                size=11, color=CGI_BODY)
            sy += Inches(0.24)
...
def slide_05_stack(prs):
    ...
    stack = [
        ("Agentic Coding",    "Claude Code CLI",    "Claude Sonnet 4.6",          "Built and iterated entirely via Claude Code in a single session"),
        ("IDE",               "Visual Studio Code", "with Claude Code extension",  "Full codebase visible; hooks into terminal for live testing"),
        ("Language",          "Python 3.11+",       "",                            "Type hints throughout; dataclasses for all structured data models"),
        ("Schema Ingestion",  "openpyxl",           "",                            "Reads TSYS TS2 workbook for REF table seed data and column definitions"),
        ("Data Generation",   "Faker (en_CA)",      "",                            "Canadian locale — SINs, postal codes, phone numbers, names"),
        ("State Persistence", "SQLite",             "stdlib sqlite3 + WAL mode",   "Single state.db file; all entities + temporal state tables"),
        ("ETL / Output",      "pandas",             "",                            "DataFrame-to-CSV for delta and snapshot output files"),
        ("Data Validation",   "pandera",            "replaced Great Expectations", "DataFrame schema validation; FK integrity checks per run"),
        ("Config",            "PyYAML",             "scenario.yaml",               "All rates, thresholds, and population sizes configurable without code changes"),
    ...
    for w, h in zip(widths, headers):
        ...
        txt(slide, h, hx + Inches(0.06), top + Inches(0.05),
            w - Inches(0.10), row_h * 0.55,
            size=12, bold=True, color=CGI_WHITE)
        hx += w + Inches(0.02)

    top += row_h * 0.65 + gap

    CHANGED = {"pandera", "pandera"}
    ...
    for i, (layer, tool, note, role) in enumerate(stack):
        ...
        for j, (w, val) in enumerate(zip(widths, [layer, tool, note, role])):
            ...
            bold  = j == 1
            txt(slide, val, rx + Inches(0.08), top + Inches(0.07),
                w - Inches(0.12), row_h - Inches(0.10),
...
def slide_06_output(prs):
    slide = blank_slide(prs)
    slide_header(slide, "Output Model",
    ...
    txt(slide, "Output Directory Structure",
        MARGIN_L + Inches(0.14), Inches(1.62),
        LEFT_W - Inches(0.20), Inches(0.32),
        size=13, bold=True, color=CGI_RED)
    ...
    for label, is_file, indent in tree:
        ...
        bold   = not is_file
        txt(slide, label,
            MARGIN_L + Inches(0.18), ty,
            LEFT_W - Inches(0.26), Inches(0.26),
    ...
    for (title, subtitle, bullets), accent in zip(cards, colors):
        ...
        txt(slide, title,
            RX + Inches(0.14), cy + Inches(0.08),
            RW - Inches(0.20), Inches(0.30),
            size=13, bold=True, color=accent)
        txt(slide, subtitle,
            RX + Inches(0.14), cy + Inches(0.40),
            RW - Inches(0.20), Inches(0.22),
            size=11, color=CGI_GREY)
        bx = cy + Inches(0.64)
        for b in bullets:
            txt(slide, f"  {b}",
                RX + Inches(0.14), bx,
...
def slide_07_samples_1(prs):
    slide = blank_slide(prs)
    slide_header(slide, "Sample Data  -  Account & Customer",
    ...
    def mini_table(title, cols, rows, x, y, w, col_widths, accent=CGI_RED):
        ...
        txt(slide, title, x + Inches(0.10), y + Inches(0.04),
            w - Inches(0.14), Inches(0.26), size=12, bold=True, color=CGI_WHITE)
        y += Inches(0.34)
        ROW_H = Inches(0.295)
        # Header
        hx = x
        for cw, ch in zip(col_widths, cols):
            ...
            txt(slide, ch, hx + Inches(0.04), y + Inches(0.04),
                cw - Inches(0.06), ROW_H - Inches(0.06),
                size=10, bold=True, color=CGI_WHITE)
            hx += cw
        y += ROW_H
        for ri, row in enumerate(rows):
            ...
            for cw, val in zip(col_widths, row):
                ...
                txt(slide, str(val), rx + Inches(0.04), y + Inches(0.04),
                    cw - Inches(0.06), ROW_H - Inches(0.06),
                    size=10, color=CGI_BODY)
                rx += cw
            y += ROW_H
    ...
    # ACCOUNT table
    acc_cols = ["account_id", "account_number", "credit_limit", "current_balance", "account_status", "days_delinquent", "risk_score"]
    acc_rows = [
    ...
    acc_widths = [Inches(0.75), Inches(1.40), Inches(0.90), Inches(1.10), Inches(0.90), Inches(0.90), Inches(0.75)]
    mini_table("ACCOUNT  (500 rows, updated daily)", acc_cols, acc_rows,
               MARGIN_L, Inches(1.56), sum(acc_widths), acc_widths, CGI_RED)
    ...
    cust_widths = [Inches(0.85), Inches(0.85), Inches(0.90), Inches(1.30), Inches(0.72), Inches(2.05), Inches(0.72)]
    mini_table("CUSTOMER  (500 rows — SIN synthetic, no real PII)", cust_cols, cust_rows,
               MARGIN_L, Inches(4.24), sum(cust_widths), cust_widths, CGI_BLUE)
...
def slide_08_samples_2(prs):
    slide = blank_slide(prs)
    slide_header(slide, "Sample Data  -  Transaction & Authorization",
    ...
    tx_widths = [Inches(0.60), Inches(0.80), Inches(1.30), Inches(0.80), Inches(0.90), Inches(2.20), Inches(0.80)]
    mini_table("TRANSACTION  (~580 new rows/day)", tx_cols, tx_rows,
               MARGIN_L, Inches(1.56), sum(tx_widths), tx_widths, CGI_RED)

    # AUTHORIZATION
    auth_cols = ["auth_id", "account_id", "amount", "response", "approval_code", "network", "channel", "decline_reason"]
    auth_rows = [
    ...
    auth_widths = [Inches(0.60), Inches(0.80), Inches(0.80), Inches(1.10), Inches(1.00), Inches(0.90), Inches(0.80), Inches(1.60)]
    mini_table("AUTHORIZATION  (~638 new rows/day  |  includes declined)", auth_cols, auth_rows,
               MARGIN_L, Inches(4.24), sum(auth_widths), auth_widths, RGBColor(0x1A,0x7A,0x3C))
...
def slide_09_samples_3(prs):
    slide = blank_slide(prs)
    slide_header(slide, "Sample Data  -  Dispute, Fraud Alert & Collection Case",
    ...
    def mini_table(title, cols, rows, x, y, w, col_widths, accent=CGI_RED):
        ...
        for ri, row in enumerate(rows):
            ...
            for cw, val in zip(col_widths, row):
                rect(slide, rx, y, cw, ROW_H, bg)
                txt(slide, str(val), rx + Inches(0.04), y + Inches(0.03),
                    cw - Inches(0.06), ROW_H - Inches(0.05),
                    size=10, color=CGI_BODY)
                rx += cw
            y += ROW_H
    ...
    d_widths = [Inches(0.80), Inches(0.65), Inches(1.30), Inches(0.80), Inches(1.00), Inches(1.20), Inches(0.90)]
    mini_table("DISPUTE  (8 total  |  progressing daily)", d_cols, d_rows,
               MARGIN_L, Inches(1.56), sum(d_widths), d_widths, CGI_RED)

    # FRAUD_ALERT
    fa_cols = ["alert_id", "account_id", "source", "type", "risk_score", "status", "action", "resolved"]
    fa_rows = [
        [1, 22, "3DS",       "LARGE_TXN",  621, "CONFIRMED",    "block_card", "2026-04-17"],
        [2, 29, "CardGuard", "LARGE_TXN",  226, "CONFIRMED",    "block_card", "2026-04-20"],
    ]
    fa_widths = [Inches(0.65), Inches(0.80), Inches(0.85), Inches(1.10), Inches(0.80), Inches(1.00), Inches(0.90), Inches(0.95)]
    mini_table("FRAUD_ALERT  (2 total  |  both confirmed, cards blocked)", fa_cols, fa_rows,
               MARGIN_L, Inches(3.84), sum(fa_widths), fa_widths, RGBColor(0xCC,0x44,0x00))

    # COLLECTION_CASE
    cc_cols = ["case_id", "account_id", "opened", "bucket", "past_due", "total_owed", "status", "next_action"]
    cc_rows = [
    ...
    cc_widths = [Inches(0.60), Inches(0.80), Inches(0.95), Inches(0.60), Inches(0.80), Inches(0.80), Inches(0.80), Inches(1.45)]
    mini_table("COLLECTION_CASE  (27 total  |  auto-opened when days_delinquent >= 30)", cc_cols, cc_rows,
               MARGIN_L, Inches(5.32), sum(cc_widths), cc_widths, CGI_BLUE)
...
def slide_10_results(prs):
    slide = blank_slide(prs)
    slide_header(slide, "7-Day Run Summary",
    ...
    txt(slide, "Daily Run Log",
        MARGIN_L + Inches(0.14), Inches(1.62),
        LEFT_W - Inches(0.20), Inches(0.30),
        size=13, bold=True, color=CGI_RED)

    day_cols  = ["Date",        "Inserts", "Txns",   "Stmts", "Updates", "Dur"]
    day_widths= [Inches(1.20),  Inches(0.80), Inches(0.70), Inches(0.60), Inches(0.90), Inches(0.50)]
    day_data  = [
    ...
    for cw, ch in zip(day_widths, day_cols):
        ...
        txt(slide, ch, hx + Inches(0.04), ty + Inches(0.05),
            cw - Inches(0.06), ROW_H - Inches(0.08),
            size=11, bold=True, color=CGI_WHITE)
        hx += cw
    ty += ROW_H
    ...
    for i, row in enumerate(day_data):
        ...
        for cw, val in zip(day_widths, row):
            ...
            txt(slide, val, rx + Inches(0.04), ty + Inches(0.05),
                cw - Inches(0.06), ROW_H - Inches(0.08),
                size=11, color=CGI_BODY)
            rx += cw
        ty += ROW_H
    ...
    txt(slide, "Cumulative State (Day 7)",
        RX, Inches(1.62), RW, Inches(0.30),
        size=13, bold=True, color=CGI_RED)
    ...
    for i, (tbl, count) in enumerate(stats):
        ...
        txt(slide, tbl, RX + Inches(0.10), sy + Inches(0.04),
            RW * 0.65, Inches(0.22), size=11, color=CGI_BODY)
        txt(slide, count, RX + RW * 0.72, sy + Inches(0.04),
            RW * 0.26, Inches(0.22), size=11, bold=True, color=CGI_RED,
    ...
    txt(slide,
        ...
        MARGIN_L + Inches(0.20), BAR_Y + Inches(0.06),
        CONTENT_W - Inches(0.40), Inches(0.34),
        size=13, bold=True, color=CGI_WHITE, align=PP_ALIGN.CENTER)
...
def slide_11_cli(prs):
    slide = blank_slide(prs)
    slide_header(slide, "CLI Reference",
    ...
    for cmd, command, desc in commands:
        ...
        txt(slide, cmd,
            MARGIN_L + Inches(0.14), top + Inches(0.08),
            Inches(0.80), Inches(0.28),
            size=12, bold=True, color=CGI_RED)

        # Command chip
        rect(slide, MARGIN_L + Inches(1.02), top + Inches(0.06),
             Inches(7.60), Inches(0.36), RGBColor(0x22,0x22,0x22))
        txt(slide, command,
            MARGIN_L + Inches(1.10), top + Inches(0.08),
            Inches(7.44), Inches(0.30),
            size=12, bold=True, color=RGBColor(0xFF,0xCC,0x44))

        txt(slide, desc,
            MARGIN_L + Inches(0.14), top + Inches(0.52),
            CONTENT_W - Inches(0.24), Inches(0.76),
            size=13, color=CGI_BODY)
...
def slide_12_nextsteps(prs):
    ...
    slide_header(slide, "Next Steps",
                 "TDGen Temporal  |  Phase 1 complete  |  TD Bank / CGI engagement")
    ...
    txt(slide, "Phase 1 Complete", MARGIN_L + Inches(0.10), Inches(1.60),
        LW - Inches(0.14), Inches(0.28), size=13, bold=True, color=CGI_WHITE)
    ...
    for item in done:
        ...
        txt(slide, "OK  " + item,
            MARGIN_L + Inches(0.12), dy + Inches(0.06),
            LW - Inches(0.18), Inches(0.34),
            size=11, color=CGI_BODY)
        dy += Inches(0.46)

    # Next column
    rect(slide, RX, Inches(1.56), RW, Inches(0.36), CGI_RED)
    txt(slide, "Phase 2 — Planned", RX + Inches(0.10), Inches(1.60),
        RW - Inches(0.14), Inches(0.28), size=13, bold=True, color=CGI_WHITE)
    ...
    for item in next_items:
        ...
        txt(slide, item,
            RX + Inches(0.12), ny + Inches(0.08),
            RW - Inches(0.18), Inches(0.40),
            size=11, color=CGI_BODY)
        ny += Inches(0.54)
...
def _v4_mini_table(slide, title, cols, rows, x, y, col_widths, accent=None):
    ...
    txt(slide, title, x + Inches(0.09), y + Inches(0.04),
        w - Inches(0.14), Inches(0.24), size=11, bold=True, color=CGI_WHITE)
    y += Inches(0.32)
    ROW_H = Inches(0.265)
    hx = x
    for cw, ch in zip(col_widths, cols):
        rect(slide, hx, y, cw, ROW_H, RGBColor(0x22, 0x22, 0x22))
        txt(slide, ch, hx + Inches(0.04), y + Inches(0.03),
            cw - Inches(0.06), ROW_H - Inches(0.05),
            size=9, bold=True, color=CGI_WHITE)
        hx += cw
    y += ROW_H
    for ri, row in enumerate(rows):
        bg = CGI_LIGHT_GREY if ri % 2 == 0 else CGI_WHITE
...
def slide_v4_01_cover(prs):
    ...
    txt(slide, "Test Data Generation",
        Inches(0.55), Inches(1.30), Inches(6.0), Inches(0.72),
        size=38, bold=True, color=CGI_BLACK)
    txt(slide, "with Claude Code",
        Inches(0.55), Inches(2.05), Inches(6.0), Inches(0.72),
        size=38, bold=True, color=CGI_BLACK)
    txt(slide, "Day-Over-Day Synthetic Test Data Generation",
        Inches(0.55), Inches(2.84), Inches(6.0), Inches(0.48),
        size=18, bold=False, color=CGI_BODY)
    ...
    ], Inches(0.55), Inches(3.58), Inches(6.0), Inches(1.20), space_pt=5)
...
def slide_v4_03_scope_text(prs):
    slide = blank_slide(prs)
    slide_header(slide, "Scope  \u2014  TSYS TS2 Credit Card Processing Schema",
    ...
    multi_para(slide, [
        ...
        ("We selected TSYS TS2 Credit Card Processing as our test schema because it is directly relevant to "
         "TD Bank\u2019s fraud management workflow and provides an enterprise-scale stress-test for TDGen.", 12, False, CGI_BODY),
        ("", 5, False, CGI_BODY),
        ...
        ("Since TS2\u2019s schema is proprietary, we constructed a proxy by synthesizing available sources from "
         ...
         "The resulting approximation is used exclusively for this POC and contains:", 12, False, CGI_BODY),
    ], MARGIN_L + Inches(0.14), Inches(1.68), CONTENT_W - Inches(0.24), Inches(2.04), space_pt=5)
    ...
    for num, label in stats:
        rect(slide, tx, ty, TILE_W, TILE_H, CGI_BLACK)
        txt(slide, num, tx, ty + Inches(0.18), TILE_W, Inches(0.90),
            size=34, bold=True, color=CGI_WHITE, align=PP_ALIGN.CENTER)
        txt(slide, label, tx, ty + Inches(1.12), TILE_W, Inches(0.50),
            size=10, color=RGBColor(0xBB, 0xBB, 0xBB), align=PP_ALIGN.CENTER)
        tx += TILE_W + TILE_GAP
...
def slide_v4_04_state_machines(prs):
    slide = blank_slide(prs)
    slide_header(slide, "Scope  \u2014  Entity State Machines",
    ...
    for name, flow, desc, color in machines:
        ...
        # Name
        txt(slide, name,
            MARGIN_L + Inches(0.12), ty + Inches(0.22),
            NAME_W - Inches(0.12), Inches(0.30),
            size=13, bold=True, color=color)
        # State flow
        txt(slide, flow,
            MARGIN_L + NAME_W, ty + Inches(0.06),
            FLOW_W - Inches(0.10), Inches(0.28),
            size=11, bold=True, color=CGI_BLACK)
        # Description
        txt(slide, desc,
            MARGIN_L + NAME_W, ty + Inches(0.36),
            FLOW_W + DESC_W - Inches(0.14), Inches(0.28),
            size=10, color=CGI_GREY)
        ty += ROW_H + ROW_G
...
def slide_v4_05_pipeline(prs):
    slide = blank_slide(prs)
    slide_header(slide, "The TDGen Daily Data Generation Pipeline",
    ...
    txt(slide,
        ...
        MARGIN_L, Inches(1.56), CONTENT_W, Inches(0.54),
        size=12, color=CGI_BODY)

    steps = [
        ("1",  "Update Accounts",      "Balance updates, delinquency aging, ACTIVE \u2192 DELINQUENT \u2192 CHARGEOFF"),
        ...
        ("5",  "Generate Authorizations","One auth per transaction + probabilistic declined-only auths  (~633/day)"),
        ("6",  "Generate Statements",  "Triggered when run_date.day == account.cycle_day  (~18/day)"),
        ("7",  "Progress Disputes",    "OPEN \u2192 INVESTIGATING \u2192 RESOLVED \u2192 CLOSED  (35 total, 30-day run)"),
        ("8",  "Progress Fraud Alerts","OPEN \u2192 UNDER_REVIEW \u2192 CONFIRMED | FALSE_POSITIVE  (11 total)"),
        ("9",  "Progress Chargebacks & Collections",
    ...
    for i, (num, heading, detail) in enumerate(steps[:8]):
        ...
        rect(slide, x, y, col_w, row_h, bg)
        rect(slide, x, y, Inches(0.04), row_h, CGI_RED)
        txt(slide, num, x + Inches(0.10), y + Inches(0.06),
            Inches(0.30), Inches(0.28), size=13, bold=True, color=CGI_RED)
        txt(slide, heading, x + Inches(0.44), y + Inches(0.06),
            Inches(1.75), Inches(0.28), size=12, bold=True, color=CGI_BLACK)
        txt(slide, detail, x + Inches(2.24), y + Inches(0.06),
            col_w - Inches(2.32), Inches(0.28), size=11, color=CGI_BODY)

    # Step 9  —  full-width row
    y9 = top + 4 * (row_h + gap)
    rect(slide, left1, y9, CONTENT_W, row_h, CGI_LIGHT_GREY)
    rect(slide, left1, y9, Inches(0.04), row_h, CGI_RED)
    txt(slide, "9", left1 + Inches(0.10), y9 + Inches(0.06),
        Inches(0.30), Inches(0.28), size=13, bold=True, color=CGI_RED)
    txt(slide, "Progress Chargebacks & Collections",
        left1 + Inches(0.44), y9 + Inches(0.06),
        Inches(3.00), Inches(0.28), size=12, bold=True, color=CGI_BLACK)
    txt(slide, "CHARGEBACK: stage advancement (4 total)  |  COLLECTION_CASE: bucket progression (27 total, 30-day run)",
        left1 + Inches(3.60), y9 + Inches(0.06),
        CONTENT_W - Inches(3.68), Inches(0.28), size=11, color=CGI_BODY)
...
def slide_v4_06_output_sample(prs):
    slide = blank_slide(prs)
    slide_header(slide, "Output Model  +  Account & Customer Sample",
    ...
    txt(slide, "OUTPUT DIRECTORY  (30-day run)",
        MARGIN_L + Inches(0.14), PANEL_TOP + Inches(0.10),
        LEFT_W - Inches(0.20), Inches(0.22),
        size=10, bold=True, color=CGI_RED)
    ...
    for text, is_file, _ind in tree:
        ...
        bold  = not is_file
        txt(slide, text,
            MARGIN_L + Inches(0.14), ty,
            LEFT_W - Inches(0.22), Inches(0.24),
    ...
    bottom = _v4_mini_table(
        ...
        acc_cols, acc_rows,
        RIGHT_X, PANEL_TOP, acc_widths, CGI_RED)

    cust_cols   = ["cust_id", "first_name", "last_name", "city", "prov", "lang"]
    cust_widths = [Inches(0.56), Inches(1.28), Inches(1.28), Inches(1.68), Inches(0.56), Inches(0.56)]
    ...
    _v4_mini_table(
        ...
        cust_cols, cust_rows,
        RIGHT_X, bottom + Inches(0.14), cust_widths, CGI_BLUE)
...
def slide_v4_07_txn_auth(prs):
    ...
    tx_widths = [Inches(0.52), Inches(0.56), Inches(1.52), Inches(0.78),
                 Inches(1.24), Inches(3.16), Inches(0.62)]
    _v4_mini_table(slide, "TRANSACTION  (~581 new rows/day  |  17,420 total after 30 days)",
                   tx_cols, tx_rows, MARGIN_L, Inches(1.56), tx_widths, CGI_RED)

    # AUTHORIZATION
    auth_cols = ["auth_id", "acct_id", "amount", "response", "approval_code", "network", "channel", "decline_reason"]
    auth_rows = [
    ...
    auth_widths = [Inches(0.56), Inches(0.56), Inches(0.78), Inches(0.92),
                   Inches(0.96), Inches(0.82), Inches(0.86), Inches(1.60)]
    _v4_mini_table(slide, "AUTHORIZATION  (~633 new rows/day  |  19,003 total  |  includes declined auths)",
                   auth_cols, auth_rows, MARGIN_L, Inches(3.90), auth_widths,
                   RGBColor(0x1A, 0x7A, 0x3C))
...
def slide_v4_08_lifecycle(prs):
    ...
    disp_widths = [Inches(0.80), Inches(0.70), Inches(1.18), Inches(0.80),
                   Inches(0.96), Inches(1.00), Inches(0.96)]
    _v4_mini_table(slide, "DISPUTE  (35 total  |  18 days with new disputes  |  multiple resolution outcomes)",
                   disp_cols, disp_rows, MARGIN_L, Inches(1.56), disp_widths, CGI_RED)

    # FRAUD_ALERT
    fa_cols = ["alert_id", "acct_id", "source", "type", "risk_score", "status", "action", "resolved"]
    fa_rows = [
    ...
    fa_widths = [Inches(0.62), Inches(0.62), Inches(0.92), Inches(1.12),
                 Inches(0.82), Inches(1.14), Inches(0.88), Inches(0.92)]
    _v4_mini_table(slide, "FRAUD_ALERT  (11 total  |  5 confirmed, 6 false-positive  |  CONFIRMED triggers card block)",
                   fa_cols, fa_rows, MARGIN_L, Inches(3.86), fa_widths,
    ...
    cc_widths = [Inches(0.62), Inches(0.62), Inches(0.94), Inches(0.62),
                 Inches(0.80), Inches(0.86), Inches(1.28)]
    _v4_mini_table(slide, "COLLECTION_CASE  (27 total  |  auto-opened at days_delinquent \u2265 30  |  buckets advance daily)",
                   cc_cols, cc_rows, MARGIN_L, Inches(5.42), cc_widths, CGI_BLUE)
...
def slide_v4_09_run_summary(prs):
    slide = blank_slide(prs)
    slide_header(slide, "30-Day Run Summary",
    ...
    for ci, rows in enumerate(col_data):
        ...
        for cw, ch in zip(sub_w, sub_h):
            txt(slide, ch, hx + Inches(0.04), TOP + Inches(0.04),
        ...
        for ri, (date, ins, txns, upd, dur) in enumerate(rows):
            bg = CGI_LIGHT_GREY if ri % 2 == 0 else CGI_WHITE
            rx = cx
            for cw, val in zip(sub_w, [date, f"{ins:,}", f"{txns:,}" if txns else "\u2014", f"{upd:,}" if upd else "\u2014", dur]):
                rect(slide, rx, ry, cw, ROW_H, bg)
                txt(slide, str(val), rx + Inches(0.04), ry + Inches(0.03),
    ...
    txt(slide, "CUMULATIVE STATE  (Day 30  \u2014  2026-04-30)",
        MARGIN_L, STATE_TOP,
    ...
    for ci, group in enumerate(groups):
        ...
        txt(slide, "Table", cx + Inches(0.06), STATE_TOP + Inches(0.04),
            nw, STATE_HDR - Inches(0.06), size=9, bold=True, color=CGI_WHITE)
        txt(slide, "Rows", cx + nw, STATE_TOP + Inches(0.04),
            rw - Inches(0.06), STATE_HDR - Inches(0.06),
            size=9, bold=True, color=CGI_WHITE, align=PP_ALIGN.RIGHT)

        ry = STATE_TOP + STATE_HDR
        for ri, (tbl, cnt) in enumerate(group):
            bg = CGI_LIGHT_GREY if ri % 2 == 0 else CGI_WHITE
            rect(slide, cx, ry, COL_W, STATE_ROW, bg)
            txt(slide, tbl, cx + Inches(0.06), ry + Inches(0.03),
                nw - Inches(0.06), STATE_ROW - Inches(0.04),
                size=9, bold=False, color=CGI_BODY)
            txt(slide, f"{cnt:,}", cx + nw, ry + Inches(0.03),
                rw - Inches(0.08), STATE_ROW - Inches(0.04),
                size=9, bold=False, color=CGI_BODY, align=PP_ALIGN.RIGHT)
            ry += STATE_ROW
    ...
    txt(slide,
        ...
        MARGIN_L, SUMMARY_Y, CONTENT_W, Inches(0.26),
        size=10, bold=False, color=CGI_GREY)
...
def slide_v4_10_cli(prs):
    ...
    for verb, cmd, desc in cmds:
        ...
        txt(slide, verb,
            MARGIN_L + Inches(0.14), ty + Inches(0.08),
            Inches(0.90), Inches(0.24),
            size=13, bold=True, color=CGI_RED)

        # Command box
        rect(slide, MARGIN_L + Inches(0.14), ty + CMD_Y_OFF,
             CONTENT_W - Inches(0.24), CMD_H,
             RGBColor(0x1A, 0x1A, 0x1A))
        txt(slide, cmd,
            MARGIN_L + Inches(0.22), ty + CMD_Y_OFF + Inches(0.03),
            CONTENT_W - Inches(0.40), CMD_H - Inches(0.04),
            size=10, bold=False, color=RGBColor(0xAA, 0xFF, 0xAA))

        txt(slide, desc,
            MARGIN_L + Inches(1.14), ty + Inches(0.06),
            CONTENT_W - Inches(1.24), Inches(0.30),
            size=11, color=CGI_BODY)
...
def slide_v4_11_nextsteps(prs):
    ...
    def phase_box(x, y, w, title, items, accent):
        ...
        txt(slide, title, x + Inches(0.14), y + Inches(0.06),
            w - Inches(0.20), Inches(0.28), size=14, bold=True, color=CGI_WHITE)
        by = y + Inches(0.44)
        for item in items:
            txt(slide, "\u25cf  " + item,
                x + Inches(0.14), by,
                w - Inches(0.22), Inches(0.26),
                size=11, color=CGI_BODY)
            by += Inches(0.30)
        return by
    ...
    phase_box(MARGIN_L, TOP, LEFT_W, "Phase 1  \u2014  Complete", phase1, RGBColor(0x1A, 0x7A, 0x3C))
...
def slide_v4_12_cgi_back(prs):
    ...
    grad_rect(slide, Inches(0), Inches(0), SLIDE_W, SLIDE_H,
              [(0.0, "E4002B"), (1.0, "8C0015")], 135)

    txt(slide,
        "Insights you can act on",
        Inches(0.90), Inches(1.20), Inches(11.53), Inches(0.80),
        size=32, bold=True, color=CGI_WHITE)
    ...
    txt(slide, body,
        Inches(0.90), Inches(2.20), Inches(11.53), Inches(3.80),
        size=16, bold=False, color=CGI_WHITE)
