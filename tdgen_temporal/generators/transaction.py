...
from tdgen_temporal.generators.field_generators import (
    _CHANNELS, _MCC_CODES, _NETWORKS, _POS_ENTRY,
    generate_card_number, random_decimal, random_narrative,
)
...
def generate_daily_transactions(
    ...
    for account in accounts:
        ...
        for _ in range(n_tx):
            ...
            if tx_type == "PAYMENT":
                amount = round(float(account.get("payment_due_amount") or 25) * rng.uniform(0.5, 2.0), 2)
            ...
            approval_no = "".join([str(rng.randint(0, 9)) for _ in range(6)]) if is_approved else None
            decline_rsn = None
            if not is_approved:
                decline_rsn = rng.choice(["insufficient_funds", "expired_card", "do_not_honour", "exceeds_limit"])
            ...
            authorizations.append({
                ...
                "available_after_auth": round(float(account.get("available_credit", 0) or 0) - amount, 2),
                "auth_type":            "standard",
...
def generate_new_disputes(
    ...
    dispute_types = ["FRAUD", "NOT_RECEIVED", "DUPLICATE", "WRONG_AMOUNT", "QUALITY", "SUBSCRIPTION"]
    disputes = []
