...
CHECKS: list[Check] = [
    Check("customer.account_id -> ACCOUNT",                   "referential", _E, "CUSTOMER",                   "Every CUSTOMER row references a valid ACCOUNT"),
    Check("card.account_id -> ACCOUNT",                       "referential", _E, "CARD",                       "Every CARD row references a valid ACCOUNT"),
    Check("account.provider_id -> PROVIDER",                  "referential", _E, "ACCOUNT",                    "Every ACCOUNT references a valid PROVIDER"),
    Check("account.product -> PRODUCT_DEFINITION",            "referential", _E, "ACCOUNT",                    "Every ACCOUNT references a valid PRODUCT_DEFINITION"),
    Check("provider.client_id -> CLIENT",                     "referential", _E, "PROVIDER",                   "Every PROVIDER references a valid CLIENT"),
    Check("transaction.account_id -> ACCOUNT",                "referential", _E, "TRANSACTION",                "Every TRANSACTION references a valid ACCOUNT"),
    Check("transaction.card_id -> CARD",                      "referential", _E, "TRANSACTION",                "Every non-null TRANSACTION.card_id references a valid CARD"),
    Check("transaction.merchant_id -> MERCHANT",              "referential", _E, "TRANSACTION",                "Every non-null TRANSACTION.merchant_id references a valid MERCHANT"),
    Check("transaction.auth_id -> AUTHORIZATION",             "referential", _E, "TRANSACTION",                "Every non-null TRANSACTION.auth_id references a valid AUTHORIZATION"),
    Check("authorization.account_id -> ACCOUNT",              "referential", _E, "AUTHORIZATION",              "Every AUTHORIZATION references a valid ACCOUNT"),
    Check("statement.account_id -> ACCOUNT",                  "referential", _E, "STATEMENT",                  "Every STATEMENT references a valid ACCOUNT"),
    Check("dispute.transaction_id -> TRANSACTION",            "referential", _E, "DISPUTE",                    "Every DISPUTE references a valid TRANSACTION"),
    Check("dispute.account_id -> ACCOUNT",                    "referential", _E, "DISPUTE",                    "Every DISPUTE references a valid ACCOUNT"),
    Check("chargeback.dispute_id -> DISPUTE",                 "referential", _E, "CHARGEBACK",                 "Every CHARGEBACK references a valid DISPUTE"),
    Check("chargeback.transaction_id -> TRANSACTION",         "referential", _E, "CHARGEBACK",                 "Every CHARGEBACK references a valid TRANSACTION"),
    Check("fraud_alert.account_id -> ACCOUNT",                "referential", _E, "FRAUD_ALERT",                "Every FRAUD_ALERT references a valid ACCOUNT"),
    Check("fraud_alert.transaction_id -> TRANSACTION",        "referential", _E, "FRAUD_ALERT",                "Every non-null FRAUD_ALERT.transaction_id references a valid TRANSACTION"),
    Check("score_record.account_id -> ACCOUNT",               "referential", _E, "SCORE_RECORD",               "Every SCORE_RECORD references a valid ACCOUNT"),
    Check("collection_case.account_id -> ACCOUNT",            "referential", _E, "COLLECTION_CASE",            "Every COLLECTION_CASE references a valid ACCOUNT"),
    Check("account_temporal_state.account_id -> ACCOUNT",     "referential", _E, "account_temporal_state",     "Every account_temporal_state row references a valid ACCOUNT"),
    Check("dispute_temporal_state.dispute_id -> DISPUTE",     "referential", _E, "dispute_temporal_state",     "Every dispute_temporal_state row references a valid DISPUTE"),
    Check("chargeback_temporal_state.chargeback_id -> CHARGEBACK", "referential", _E, "chargeback_temporal_state", "Every chargeback_temporal_state row references a valid CHARGEBACK"),
    Check("account has no customer",                         "referential", _E, "ACCOUNT",                    "Every ACCOUNT has at least one matching CUSTOMER row"),
    Check("account has no temporal state",                   "referential", _E, "ACCOUNT",                    "Every ACCOUNT has a matching account_temporal_state row"),
]
